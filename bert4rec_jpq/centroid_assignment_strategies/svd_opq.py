...
class SVDAssignmentStrategy_opq(CentroidAssignmentStragety):
    ...
    def assign(self, train_users=None, svd_embeddings=None):
        ...
        # reconstructed_embeddings = pq.decode(item_codes) # exclude padding row
        # distortion = np.mean(np.sum((svd_embeddings - reconstructed_embeddings) ** 2, axis=1))
        # print(f"Quantization distortion: {distortion:.6f}")
