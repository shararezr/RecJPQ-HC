...
from centroid_assignment_strategies.svd_opqfixed import SVDAssignmentStrategy_opq
...
class ItemCodeLayer(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer(
            "item_codes",
            torch.zeros(num_items+1, self.item_code_bytes, dtype=torch.long)
        )
...
class StandardItemEmbedding(nn.Module):
    """
    Standard nn.Embedding with the same interface as ItemCodeLayer.
    Drop-in replacement for pure SASRec without JPQ.
    """

    def __init__(self, embedding_size, num_items, padding_idx=None):
        super().__init__()
        self.embedding_size = embedding_size
        self.num_items = num_items
        self.padding_idx = padding_idx
    ...
    def score_sequence_items(self, seq_emb, target_ids):
        ...
        returns:   [B, L, N]
        """
        target_emb = self.embedding(target_ids.long())  # [B, L, N, D]
        scores = torch.einsum("bld,blnd->bln", seq_emb, target_emb)
        return scores
    ...
    def score_all_items(self, seq_emb):
        ...
        returns: [B, num_items]
        """
        item_emb = self.embedding.weight[:self.num_items]  # [num_items, D]
